...
for delphes_file in string_to_list(delphes_file_list):
	...
	ntuple_trees = create_ntuple_trees(ptcl_var_to_wrt, flatten_vars, sort_each_var)
		...
		if particle_var_veto(evt, 'photon', 'energy',True, 'highest < 1 GeV'):
			...
			if opposite_charge_pairs != 0:
				...
				write_to_ntuple_tree(delphes_file, ntuple_trees, evt, 
				                     final_cand, var_to_wrt, flatten_vars, sort_each_var)
			else:
