...
for i, delphes_file in enumerate(delphes_file_list):
	...
	for ievt, evt in enumerate(evt_chain):
		...
		if (((num_electron == 1 and num_positron == 1) and 
			...
			num_mumu_evts += 1
		      ...
		      (num_tau      == 0 or num_antitau  == 0)):
			num_no_id_evts += 1
	print 'number of ee events: ', num_ee_evts * w[i]
	print 'number of mumu events: ', num_mumu_evts * w[i]
	print 'number of double channel events: ', num_double_channel_evts * w[i]
	print 'number of no id events: ', num_no_id_evts * w[i]
