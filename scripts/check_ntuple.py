...
'4f-2mu2l' :('ee4lepton_muon:electron-muon:pt-eta-phi-cos_theta-'
             'alpha-p_mag-m_inv-m_rec-p_mag_missing-cos_theta_p_missing.root'),
'4f-2mu2q' :('ee4lepquark:electron-muon:pt-eta-phi-cos_theta-'
             'alpha-p_mag-m_inv-m_rec-p_mag_missing-cos_theta_p_missing.root')
}
...
sig_filepath = {}
bkg_filepath = {}
for i, f in sig_files.items(): 
	sig_filepath[i] = ntuple_path + sig_files[i] 
for i, f in bkg_files.items(): 
	bkg_filepath[i] = ntuple_path + bkg_files[i]

sig_trees = get_ntuple_tree(sig_filepath, channels)
bkg_trees = get_ntuple_tree(bkg_filepath, channels)
sig_w = get_ntuple_weight(sig_filepath, lumi)
bkg_w = get_ntuple_weight(bkg_filepath, lumi)
#bkg = {}
